            ...
            try
            {
                if (int.TryParse(dgvSuKien.Rows[dong].Cells[0].Value.ToString(), out maSuKien))
                {
                    ...
                    if (dr == DialogResult.Yes)
                    {
                        try
                        {
                            string query = "delete Luatsukien where MaSuKien = '" + maSuKien+"'";
                            bool result = objXldl.ThucHienCauLenh(query);
                            objXldl.ThucHienCauLenh("delete SuKien where MaSuKien ='" + maSuKien + "'");
